...
use vfs_core::models::*;
use vfs_storage::{CachedDatabase, FileStore}; // 改用 CachedDatabase
...
pub async fn login(
    ...
    // 生成 JWT
    let jwt_service = crate::auth::JwtService::new(&vfs_core::config::Config::default().auth);
...
async fn broadcast_merge_result(state: &AppState, user_id: Uuid, change: &SyncLog) {
    ...
    let packet = vfs_sync::packet::SyncPacket {
    ...
    let message = vfs_sync::packet::WsMessage::SyncPacket {
